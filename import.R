...
sqf.data.full <- foreach(year=2008:2016, .combine='rbind') %dopar% {
  ...
  if (year %in% 2013:2016) {
    names(this.data)[names(this.data) == "dettypCM"] <- "dettypcm"
    names(this.data)[names(this.data) == "lineCM"] <- "linecm"
    names(this.data)[names(this.data) == "detailCM"] <- "detailcm"
  }
  
  # Insert forceuse column if not in dataset
  ...
  if (year==2014){
    ...
    # reasons for frisk, stop and search
    for (i in which(colnames(this.data)=="ac_rept") : which(colnames(this.data)=="sb_other")) {
      this.data[[i]] <- recode.1NA(this.data[[i]])
    }
    
    # two other frisk variables
